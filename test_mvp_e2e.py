        ...
        try:
            ...
            for step in steps:
                ...
                if agent == 'EvaluationAgent':
                    ...
                    print(f"  - {agent} (迭代 {iteration}): {status} - 评分 {score}/10")
                else:
            ...
            # 2. 迭代次数可控
            max_iterations = 3
            iterations_controlled = result.get('total_iterations', 0) <= max_iterations
            ...
            for step in steps:
                if step.get('agent') == 'EvaluationAgent':
                    improvements = step.get('result', {}).get('improvements', '')
                    if improvements:
